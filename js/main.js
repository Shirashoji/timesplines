document.addEventListener("DOMContentLoaded", function (e) {
  ...
  FilterUtil.setFilterDisplayArea(
    0,
    0,
    mSvg.attr("width"),
    mSvg.attr("height"),
  ...
  let mMainOverlay = mVizOverlayLayer
    .append("rect")
  ...
  let mLinkLine = mInteractionLayer
    .append("line")
  ...
  window.addEventListener("resize", () => {
    mSvg.attr("width", window.innerWidth).attr("height", window.innerHeight);
    mMainOverlay
      .attr("width", window.innerWidth)
      .attr("height", window.innerHeight);
  });
  ...
  mDrawerController.setDrawerResizedCallback((width) => {
    ...
    if (mLensController.getCurrentTimelineId()) {
      showLensView(
        mLensController.getCurrentTimelineId(),
        mLensController.getCurrentCenterPercent(),
      );
    }
  });
  $("#data-drawer")
  ...
  let mLensController = new LensController(
    mLensSvg,
    mModelController,
    pushVersion,
    modelUpdated,
  ...
  let mSelectionController = new SelectionController(
    mVizLayer,
    mVizOverlayLayer,
    mInteractionLayer,
  );
  mSelectionController.setDragStartCallback((timelineId, pointerEvent) => {
    let coords = screenToSvgCoords({
      x: pointerEvent.clientX,
      y: pointerEvent.clientY,
    });
    return coords;
  });
  mSelectionController.setLineModifiedCallback(
    (timelineId, points, newPoints) => {
      mModelController.updateTimelinePoints(
        timelineId,
        [{ points }],
        [{ points: newPoints }],
      );
  ...
  let mLineViewController = new LineViewController(
    mVizLayer,
    mVizOverlayLayer,
    mInteractionLayer,
  );
  mLineViewController.setLineDragStartCallback((timelineId, pointerEvent) => {
    if (mMode == Mode.SELECTION) {
      mSelectionController.onTimelineDragStart(
        timelineId,
        screenToSvgCoords({ x: pointerEvent.clientX, y: pointerEvent.clientY }),
      );
    }
    if (mMode == Mode.PIN) {
      ...
      mDragStartPosition = screenToSvgCoords({
        x: pointerEvent.clientX,
        y: pointerEvent.clientY,
      });
      ...
      mModelController.imageBindingToLineBinding(
        timelineId,
        mSelectedImageBindingId,
        linePoint,
      );
  ...
  mLineViewController.setLineDragCallback((timelineId, coords) => {
    if (mMode == Mode.PIN) {
      ...
      pinDrag(
        timeline,
        mDraggingTimePin,
        PathMath.getClosestPointOnPath(coords, timeline.points).percent,
      );
    } else if (mMode == Mode.LENS) {
      let linePoint = PathMath.getClosestPointOnPath(
        coords,
    ...
    if (mMode == Mode.PIN) {
      ...
      if (mModelController.getModel().hasTimeMapping(timelineId)) {
        let time = mModelController
          .getModel()
          .mapLinePercentToTime(timelineId, linePoint.percent);
        mModelController.addBoundTextRow(
          timelineId,
          DataUtil.getFormattedDate(time),
          time,
        );
      } else {
        let timePin = new DataStructs.TimePin(linePoint.percent);
        timePin.timePercent = mModelController
          .getModel()
          .mapLinePercentToTime(timelineId, linePoint.percent);
      ...
      FileHandler.getImageFile().then(({ imageData, width, height }) => {
        ...
        if (mModelController.getModel().hasTimeMapping(timelineId)) {
          let time = mModelController
            .getModel()
            .mapLinePercentToTime(timelineId, linePoint.percent);
          mModelController.addBoundImage(
            timelineId,
            imageData,
            width,
            height,
      ...
      let cellBindingData = mModelController
        .getModel()
        .getCellBindingData(timelineId);
      let newText = cellBindingData.filter(
        (b) =>
          newCellIds.includes(b.cellBinding.id) &&
          b.dataCell.getType() == DataTypes.TEXT,
      );
      if (newText.length > 0) {
        ...
        boundingBoxes = boundingBoxes.filter((b) =>
          allCellBindingIds.includes(b.cellBindingId),
        );

        let newOffsets = DataUtil.layoutText(
          newText.map((cbd) => cbd.cellBinding),
          boundingBoxes,
        );
        newOffsets.forEach((d) =>
          mModelController.updateTextOffset(d.cellBindingId, d.offset),
        );
      }
      ...
      let timeline = mModelController.getModel().getTimelineById(timelineId);
      let points1 = [];
      let points2 = timeline.points.map((p) =>
        Object.assign({}, { x: p.x, y: p.y }),
      );
  ...
  mLineViewController.setPointerEnterCallback((event, timelineId) => {
    if (
      ...
      mDataTableController.highlightCells(
        mModelController.getModel().getTimelineHighlightData(timelineId),
      );
      FilterUtil.applyShadowFilter(
        mVizLayer.selectAll('[timeline-id="' + timelineId + '"]'),
      );
    } else if (mMode == Mode.LINK) {
      FilterUtil.applyShadowFilter(
        mVizLayer.selectAll('[timeline-id="' + timelineId + '"]'),
  ...
  let mStrokeController = new StrokeController(
    mVizLayer,
    mVizOverlayLayer,
    mInteractionLayer,
  );
  ...
  mStrokeController.setDragEndCallback((strokeId, coords) => {
    if (mMode == Mode.SELECTION) {
      if (mModelController.isCanvasStroke(strokeId)) {
        let stroke = mModelController.getModel().getStrokeById(strokeId);
        let diff = MathUtil.subtractAFromB(mDragStartPosition, coords);
        mModelController.updateStrokePoints(
          strokeId,
          stroke.points.map((p) => {
            p.xValue += diff.x;
            p.lineDist += diff.y;
      ...
      setColorBrushColor(
        mModelController.getModel().getStrokeById(strokeId).color,
      );
    } else if (mMode == Mode.COLOR_BUCKET_EYEDROPPER) {
      setColorBucketColor(
        mModelController.getModel().getStrokeById(strokeId).color,
  ...
  let mTimePinController = new TimePinController(
    mVizLayer,
    mVizOverlayLayer,
    mInteractionLayer,
  );

  mTimePinController.setDragStartCallback((event, timePin) => {
    /* don't need to do anything here. */
  });
  mTimePinController.setDragCallback((coords, timePin) => {
    let timeline = mModelController
      .getModel()
      .getTimelineForTimePin(timePin.id);
    let projectedPoint = PathMath.getClosestPointOnPath(
      coords,
      timeline.points,
    );
  ...
  mTimePinController.setPointerEnterCallback((event, timePin) => {
    ...
    let message = timePin.timeStamp
      ? DataUtil.getFormattedDate(timePin.timeStamp)
      : "時間の割合: " + Math.round(timePin.timePercent * 100) + "%";

    let timeCell = mModelController.getModel().getTimeCellForPin(timePin.id);
    if (timeCell) {
      if (timeCell.isValid()) {
        console.error(
          "Bad state. Valid time linked to pin.",
          timeCell,
          timePin,
        );
      } else if (timeCell.getValue()) {
        message =
          "<div>" + message + "<div></div>" + timeCell.getValue() + "</div>";
      }
    }
  ...
  let mTextController = new TextController(
    mVizLayer,
    mVizOverlayLayer,
    mInteractionLayer,
  );
  mTextController.setPointerEnterCallback((e, cellBindingData) => {
    mDataTableController.highlightCells(
      mModelController
        .getModel()
        .getCellBindingHighlightData(cellBindingData.cellBinding),
    );
  });
  ...
  mTextController.setDragStartCallback((cellBindingData, pointerEvent) => {
    ...
    if (mMode == Mode.TEXT || mMode == Mode.SELECTION) {
      ...
      cellBindingData.cellBinding.offset = MathUtil.subtractAFromB(
        linePoint,
        coords,
      );
  ...
  mTextController.setDragCallback((cellBindingData, startPos, coords) => {
    if (mMode == Mode.TEXT || mMode == Mode.SELECTION) {
      ...
      cellBindingData = cellBindingData.copy();
      let offset = MathUtil.addAToB(
        cellBindingData.cellBinding.offset,
        MathUtil.subtractAFromB(startPos, coords),
      );
      ...
      if (mDraggingTimePinSettingTime) {
        if (mModelController.getModel().hasTimeMapping(timeline.id)) {
          mDraggingTimePin.timeStamp = mModelController
            .getModel()
            .mapLinePercentToTime(timeline.id, linePoint.percent, false);
        } else {
          mDraggingTimePin.timePercent = mModelController
            .getModel()
  ...
  mTextController.setDragEndCallback((cellBindingData, startPos, coords) => {
    if (mMode == Mode.TEXT || mMode == Mode.SELECTION) {
      ...
      let orginalBinding = mModelController
        .getModel()
        .getCellBindingById(cellBindingData.cellBinding.id);
      let offset = MathUtil.addAToB(
        orginalBinding.offset,
        MathUtil.subtractAFromB(startPos, coords),
      );
      ...
      if (!cellBindingData.timeCell.isValid()) {
        mModelController.updateTimePinBinding(
          cellBindingData.cellBinding.id,
          mDraggingTimePin.id,
        );
      }
      ...
      mModelController.setCellBindingColor(
        cellBindingData.cellBinding.id,
        mBucketColor,
      );
      pushVersion();
  ...
  mTextController.setDoubleClickCallback(
    (cellId, text, x, y, height, width) => {
  ...
  let mImageController = new ImageController(
    mVizLayer,
    mVizOverlayLayer,
    mInteractionLayer,
  );
  mImageController.setDragStartCallback((imageBindingData, pointerEvent) => {
    ...
    if (mMode == Mode.IMAGE || mMode == Mode.SELECTION) {
      ...
      if (!imageBindingData.isCanvasBinding) {
        position = PathMath.getPositionForPercent(
          imageBindingData.timeline.points,
          imageBindingData.linePercent,
        );
      }
      ...
      imageBindingData.imageBinding.offset = MathUtil.subtractAFromB(
        linePoint,
        coords,
      );
  ...
  mImageController.setDragCallback((imageBindingData, startPos, coords) => {
    if (mMode == Mode.IMAGE || mMode == Mode.SELECTION) {
      ...
      let offset = MathUtil.addAToB(
        imageBindingData.imageBinding.offset,
        MathUtil.subtractAFromB(startPos, coords),
      );
  ...
  mImageController.setDragEndCallback((imageBindingData, startPos, coords) => {
    if (mMode == Mode.IMAGE || mMode == Mode.SELECTION) {
      ...
      if (!MathUtil.pointsEqual(startPos, coords)) {
        ...
        mModelController.updateImageOffset(
          imageBindingData.imageBinding.id,
          offset,
        );
  ...
  let mDataPointController = new DataPointController(
    mVizLayer,
    mVizOverlayLayer,
    mInteractionLayer,
  );
  mDataPointController.setPointDragStartCallback(
    (cellBindingData, pointerEvent) => {
      let coords = screenToSvgCoords({
        x: pointerEvent.clientX,
        y: pointerEvent.clientY,
      });
  ...
  mDataPointController.setPointDragCallback((cellBindingData, coords) => {
    if (mMode == Mode.PIN) {
      ...
      if (mDraggingTimePinSettingTime) {
        if (mModelController.getModel().hasTimeMapping(timeline.id)) {
          timePin.timeStamp = mModelController
            .getModel()
            .mapLinePercentToTime(timeline.id, linePoint.percent, false);
        } else {
          mDraggingTimePin.timePercent = mModelController
  ...
  mDataPointController.setAxisDragStartCallback(
    (axis, controlNumber, event) => {
      if (mMode == Mode.SELECTION) {
  ...
  mDataPointController.setAxisDragCallback((axis, controlNumber, coords) => {
    if (mMode == Mode.SELECTION) {
      ...
      if (controlNumber == null) {
        let closestPoint = PathMath.getClosestPointOnPath(
          coords,
          timeline.points,
        );
        axis.linePercent = closestPoint.percent;
      } else {
        let origin = PathMath.getPositionForPercent(
          timeline.points,
          axis.linePercent,
        );
        let normal = PathMath.getNormalForPercent(
          timeline.points,
          axis.linePercent,
        );
        let newPosition = MathUtil.projectPointOntoVector(
          coords,
          normal,
          origin,
        );
      ...
      if (boundData.length == 0) {
        console.error(
          "Bad state. Should not display a axis that has no data.",
          axis.id,
        );
        return;
      }
  ...
  mDataPointController.setAxisDragEndCallback((axis, controlNumber, coords) => {
    if (mMode == Mode.SELECTION) {
      ...
      mModelController.updateAxisPosition(
        axis.id,
        axis.dist1,
        axis.dist2,
        axis.linePercent,
  ...
  function setDragPinForBindingDrag(bindingData, linePoint, isImage = false) {
    ...
    let timePinId = isImage
      ? bindingData.imageBinding.timePinId
      : bindingData.cellBinding.timePinId;
    let timeIsValid = isImage
      ? bindingData.imageBinding.timeStamp
      : bindingData.timeCell.isValid();
    let time = isImage
      ? bindingData.imageBinding.timeStamp
      : bindingData.timeCell.getValue();

    let timePin;
    if (timeIsValid) {
      timePin = bindingData.timeline.timePins.find(
        (pin) => pin.timeStamp == time,
      );
    } else if (timePinId) {
      timePin = bindingData.timeline.timePins.find(
        (pin) => pin.id == timePinId,
      );
    }
    ...
    if (!timePin) {
      ...
      let hasTimeMapping = mModelController
        .getModel()
        .hasTimeMapping(bindingData.timeline.id);
      if (timeIsValid) {
        timePin.timeStamp = time;
      } else if (hasTimeMapping) {
        timePin.timeStamp = mModelController
          .getModel()
          .mapLinePercentToTime(
            bindingData.timeline.id,
            linePoint.percent,
            false,
          );
      }
  ...
  function showAxisContextMenu(axis, timeline) {
    let basePose = PathMath.getPositionForPercent(
      timeline.points,
      axis.linePercent,
    );
    let normal = PathMath.getNormalForPercent(
    ...
    let pos1 = MathUtil.getPointAtDistanceAlongVector(
      axis.dist1,
      normal,
      basePose,
    );
    let pos2 = MathUtil.getPointAtDistanceAlongVector(
      axis.dist2,
      normal,
      basePose,
    );
  ...
  let mLineDrawingController = new LineDrawingController(
    mVizLayer,
    mVizOverlayLayer,
    mInteractionLayer,
  );
  mLineDrawingController.setDrawFinishedCallback(
    (newPoints, color, startPointLineId = null, endPointLineId = null) => {
      if (startPointLineId == null && endPointLineId == null) {
        ...
        mModelController.extendTimeline(
          startPointLineId ? startPointLineId : endPointLineId,
          newPoints,
          startPointLineId != null,
        );
  ...
  let mColorBrushController = new ColorBrushController(
    mVizLayer,
    mVizOverlayLayer,
    mInteractionLayer,
  );
  ...
  let mEraserController = new EraserController(
    mVizLayer,
    mVizOverlayLayer,
    mInteractionLayer,
  );
  mEraserController.setEraseCallback((canvasMask) => {
    ...
    if (mMode == Mode.ERASER_TEXT || mMode == Mode.ERASER) {
      ...
      let boundingBoxes = mTextController.getTextBoundingBoxes();
      let maskedCellBindingIds = DataUtil.getMaskedText(
        canvasMask,
        boundingBoxes,
      );
    ...
    if (mMode == Mode.ERASER_TIMELINE || mMode == Mode.ERASER) {
      let segmentsData = DataUtil.getMaskedTimelines(
        canvasMask,
        mModelController.getModel(),
      );

      let deletedTimelines = segmentsData
        .filter(
          (d) =>
            d.segments.length == 1 &&
            d.segments[0].label == SEGMENT_LABELS.DELETED,
        )
        .map((d) => d.id);
      ...
      brokenTimelines.forEach((d) =>
        mModelController.breakTimeline(d.id, d.segments),
      );
      if (deletedTimelines.length > 0 || brokenTimelines.length > 0)
        changeMade = true;
    }
    if (mMode == Mode.ERASER_STROKE || mMode == Mode.ERASER) {
      ...
      strokeFragementData.forEach(({ strokeData, fragments }) => {
        if (mModelController.isCanvasStroke(strokeData.id)) {
          ...
          if (!timeline) {
            console.error("No timeline for timeline stroke!", strokeData);
            return;
          }
          fragments.forEach((fragment) => {
      ...
      mModelController.deleteStrokes(
        strokeFragementData.map((s) => s.strokeData.id),
      );
      if (strokeFragementData.length > 0) changeMade = true;
    }
    if (mMode == Mode.ERASER_POINT || mMode == Mode.ERASER) {
      let maskedCellBindingIds = DataUtil.getMaskedDataPoints(
        canvasMask,
        mModelController.getModel(),
      );
      mModelController.deleteCellBindings(maskedCellBindingIds);
    ...
    if (mMode == Mode.ERASER_IMAGE || mMode == Mode.ERASER) {
      let erasedImageIds = DataUtil.getMaskedImages(
        canvasMask,
        mModelController.getModel(),
      );
    ...
    if (mMode == Mode.ERASER_PIN) {
      let erasedPinIds = DataUtil.getMaskedPins(
        canvasMask,
        mModelController.getModel(),
      );
  ...
  let mDeformController = new DeformController(
    mVizLayer,
    mVizOverlayLayer,
    mInteractionLayer,
  );
  ...
  mDeformController.setLineModifiedCallback((data) => {
    data.forEach((d) =>
      mModelController.updateTimelinePoints(d.id, d.oldSegments, d.newSegments),
    );
  ...
  let mSmoothController = new SmoothController(
    mVizLayer,
    mVizOverlayLayer,
    mInteractionLayer,
  );
  ...
  mDataTableController.setOnSelectionCallback(
    (yTop, yBottom, isFirstColOnly) => {
      if (isFirstColOnly) {
        $("#link-button-div").hide();
      } else {
        ...
        let minPos = 10;
        let position =
          (yTop + yBottom) / 2 - $("#link-button-div").height() / 2 - 10;

        $("#link-button-div").css(
          "top",
          Math.min(maxPos, Math.max(minPos, position)),
        );
  ...
  mResizeController.setDragStartCallback((pointerEvent, bounds) => {
    mResizingImageBindingData = mModelController
      .getModel()
      .getImageBindingDataById(mSelectedImageBindingId);
  ...
  mResizeController.setDragCallback((dragCoords, startPos, newBounds) => {
    if (mSelectedImageBindingId) {
      ...
      if (mResizingImageBindingData.startBounds.x1 != newBounds.x1) {
        imageBindingData.imageBinding.offset.x =
          imageBindingData.imageBinding.offset.x + dragCoords.x - startPos.x;
      }

      if (mResizingImageBindingData.startBounds.y1 != newBounds.y1) {
        imageBindingData.imageBinding.offset.y =
          imageBindingData.imageBinding.offset.y + dragCoords.y - startPos.y;
      }
  ...
  mResizeController.setDragEndCallback((dragCoords, startPos, newBounds) => {
    if (mSelectedImageBindingId) {
      ...
      mModelController.updateImageSize(
        mSelectedImageBindingId,
        offset,
        height,
        width,
  ...
  $(document).on("pointerdown", function (event) {
    ...
    if (
      $(event.target).closest("#text-context-menu-div").length === 0 &&
    ...
    if (
      $(event.target).closest("#image-context-menu-div").length === 0 &&
    ...
    if (
      $(event.target).closest("#axis-context-menu-div").length === 0 &&
    ...
    if (
      $(event.target).closest("#color-picker-div").length === 0 &&
      $(event.target).closest("#color-brush-button-color-picker").length ===
        0 &&
      $(event.target).closest("#color-bucket-button-color-picker").length ===
        0 &&
  ...
  $(document).on("pointerup", function (e) {
    ...
    return Promise.all([mEraserController.onPointerUp(coords)]);
  });
  ...
  $(document).keydown(function (e) {
    ...
    if (
      ((e.ctrlKey || e.metaKey) && /* y */ e.which == 89) ||
      ((e.ctrlKey || e.metaKey) && e.shiftKey && /* z */ e.which == 90)
    ) {
  ...
  function deleteSelected() {
    let selectedCount = [
      mSelectedCellBindingId,
      mSelectedImageBindingId,
      mSelectedAxisId,
    ].reduce((count, item) => (item == null ? count : count + 1), 0);
    if (selectedCount > 1) {
      console.error("Multiple selected items!", [
        mSelectedCellBindingId,
        mSelectedImageBindingId,
        mSelectedAxisId,
      ]);
  ...
  function screenToSvgCoords(screenCoords) {
    ...
    if (
      isNaN(parseInt(svgViewportPos.x)) ||
      isNaN(parseInt(svgViewportPos.y))
    ) {
    ...
    if (
      isNaN(parseInt(mViewTransform.x)) ||
      isNaN(parseInt(mViewTransform.y))
    ) {
    ...
    return {
      x:
        (screenCoords.x - svgViewportPos.x - mViewTransform.x) /
        mViewTransform.scale,
      y:
        (screenCoords.y - svgViewportPos.y - mViewTransform.y) /
        mViewTransform.scale,
    };
  }
  ...
  function svgCoordsToScreen(svgCoords) {
    ...
    return {
      x:
        svgCoords.x * mViewTransform.scale +
        svgViewportPos.x +
        mViewTransform.x,
      y:
        svgCoords.y * mViewTransform.scale +
        svgViewportPos.y +
        mViewTransform.y,
    };
  }
  ...
  function setViewToTransform() {
    ...
    let bottomRight = screenToSvgCoords({
      x: mSvg.attr("width"),
      y: mSvg.attr("height"),
    });
    FilterUtil.setFilterDisplayArea(
  ...
  function pinDrag(timeline, timePin, linePercent) {
    ...
    let timelineHasMapping = mModelController
      .getModel()
      .hasTimeMapping(timeline.id);
    let timeAttribute = timelineHasMapping ? "timeStamp" : "timePercent";

    let tempPins = DataUtil.filterTimePinByChangedPin(
      timeline.timePins,
      changedPin,
      timeAttribute,
    );
    ...
    let timeBindingValues = DataUtil.filterTimePinByChangedPin(
      mModelController.getModel().getTimeBindingValues(timeline),
      changedPin,
      timeAttribute,
    );

    mLineViewController.drawSingleTimeline(
      timeline,
      timeBindingValues,
      timeAttribute,
    );
  ...
  function modelUpdated() {
    ...
    $("body").css(
      "background-color",
      mModelController.getModel().getCanvas().color,
    );
  }
  ...
  setupButtonTooltip("#undo-button", "最後の操作を元に戻す");
  ...
  setupButtonTooltip("#redo-button", "元に戻した操作をやり直す");
  ...
  setupButtonTooltip("#upload-button", "以前の作業を読み込むメニューを表示");
  ...
  setupButtonTooltip(
    ...
    "ワークスペースフォルダからビジュアライゼーションを選択して読み込む",
  );
  ...
  setupButtonTooltip(
    "#upload-button-json",
    "以前にダウンロードしたJSONファイルで現在のビジュアライゼーションを置き換える",
  );
  ...
  setupButtonTooltip(
    "#download-button-folder",
    "このビジュアライゼーション用のワークスペースフォルダを設定",
  );
  ...
  setupButtonTooltip(
    ...
    "あなたの画像をJSONファイルにパッケージ化して後でアップロードできるようにする",
  );
  ...
  $("#download-button-svg").on("click", () => {
    ...
    exportSVG.append(function () {
      return viz.node();
    });
    FileHandler.downloadSVG(exportSVG.node());
    log(LogEvent.WRITE_SVG, "");
  });
  setupButtonTooltip("#download-button-svg", "ビジュアライゼーションをSVGとしてダウンロード");

  $("#download-button-png").on("click", async () => {
    let canvas = await DataUtil.vizToCanvas(
      mVizLayer,
      mModelController.getModel().getCanvas().color,
    );
  ...
  setupButtonTooltip("#download-button-png", "ビジュアライゼーションをPNGとしてダウンロード");
  // ---------------
  setupModeButton("#line-drawing-button", Mode.LINE_DRAWING, () => {
    mLineDrawingController.setActive(true);
  });
  setupButtonTooltip("#line-drawing-button", "タイムラインを描画");
  ...
  setupSubModeButton(
    "#line-drawing-button",
    "-eyedropper",
    Mode.LINE_DRAWING_EYEDROPPER,
    setEyeDropperActive,
  );
  setupButtonTooltip(
    "#line-drawing-button-eyedropper",
    "色付け可能なものから色をコピー",
  );
  ...
  setupButtonTooltip(
    "#line-drawing-button-color-picker",
    "タイムラインの色を選択",
  );
  ...
  setupButtonTooltip("#line-manipulation-button-deform", "タイムラインを変形する");
  setupSubModeButton(
    "#line-manipulation-button",
    "-smooth",
    Mode.SMOOTH,
  ...
  setupButtonTooltip("#line-manipulation-button-smooth", "タイムラインを平滑化する");

  setupModeButton("#scissors-button", Mode.SCISSORS, () => {
    mLineViewController.setActive(true);
  });
  setupButtonTooltip("#scissors-button", "タイムラインを分割する");
  ...
  setupButtonTooltip(
    "#toggle-timeline-style-button",
    "利用可能なタイムラインスタイルを切り替える",
  );
  // ---------------
  ...
  setupSubModeButton(
    "#color-brush-button",
    "-eyedropper",
    Mode.COLOR_BRUSH_EYEDROPPER,
    setEyeDropperActive,
  );
  setupButtonTooltip(
    "#color-brush-button-eyedropper",
    "色付け可能なものから色をコピー",
  );
  ...
  setupButtonTooltip("#color-brush-button-color-picker", "ブラシの色を選択");
  ...
  $("#increase-font-size-button").on("click", () => {
    ...
    let cellBinding = mModelController
      .getModel()
      .getCellBindingById(mSelectedCellBindingId);
    if (!cellBinding) {
      console.error(
        "Bad State! Cell binding not found",
        mSelectedCellBindingId,
      );
      return;
    }

    mModelController.setFontSize(
      mSelectedCellBindingId,
      Math.min(cellBinding.fontSize + 4, 64),
    );
    pushVersion();
    modelUpdated();
  });
  $("#decrease-font-size-button").on("click", () => {
    ...
    mModelController.setFontSize(
      mSelectedCellBindingId,
      Math.max(cellBinding.fontSize - 4, 4),
    );
    pushVersion();
    modelUpdated();
  });
  $("#delete-text-button").on("click", deleteSelected);
  setupButtonTooltip("#delete-text-button", "このテキストをビジュアルからリンク解除");

  setupModeButton("#image-button", Mode.IMAGE, () => {
    mLineViewController.setActive(true);
    mImageController.setActive(true);
  });
  setupButtonTooltip("#image-button", "ビジュアライゼーションに画像を追加");
  ...
  setupButtonTooltip("#image-unlink-button", "画像をラインから切り離す");
  $("#delete-image-button").on("click", deleteSelected);
  setupButtonTooltip("#delete-image-button", "この画像を削除");
  ...
  setupModeButton("#image-link-button", Mode.IMAGE_LINK, () => {
    ...
    mLinkingBinding = mModelController
      .getModel()
      .getImageBindingById(mSelectedImageBindingId);
    mLineViewController.setActive(true);
  });
  setupButtonTooltip("#image-link-button", "画像をラインに添付");
  ...
  $("#image-time-edit-button").on("click", (event) => {
    ...
    let imageBinding = mModelController
      .getModel()
      .getImageBindingById(mSelectedImageBindingId);
    if (!imageBinding) {
    ...
    let time = imageBinding.timeStamp
      ? DataUtil.getFormattedDate(imageBinding.timeStamp)
      : "";
    ...
    mTextInputBox.setIsValidCallback((text) => {
      return (
        text && (!isNaN(new Date(text)) || !isNaN(new Date(parseInt(text))))
      );
    });
  });
  setupButtonTooltip(
    "#image-time-edit-button",
    "画像に割り当てられた時間を編集",
  );
  ...
  setupButtonTooltip("#pin-button", "タイムライン上に時間ピンを作成・移動");
  ...
  setupButtonTooltip("#selection-button", "アイテムを選択して移動");
  ...
  setupButtonTooltip("#eraser-button", "すべてのものを消去！");
  ...
  setupSubModeButton(
    "#eraser-button",
    "-timeline",
    Mode.ERASER_TIMELINE,
    () => {
      mEraserController.setActive(true);
    },
  );
  setupButtonTooltip("#eraser-button-timeline", "タイムラインのみを消去");
  setupSubModeButton("#eraser-button", "-stroke", Mode.ERASER_STROKE, () => {
    mEraserController.setActive(true);
  });
  setupButtonTooltip("#eraser-button-stroke", "ストロークのみを消去");
  setupSubModeButton("#eraser-button", "-point", Mode.ERASER_POINT, () => {
    mEraserController.setActive(true);
  });
  setupButtonTooltip("#eraser-button-point", "ポイントのみを消去");
  setupSubModeButton("#eraser-button", "-text", Mode.ERASER_TEXT, () => {
    mEraserController.setActive(true);
  });
  setupButtonTooltip("#eraser-button-text", "テキストのみを消去");
  setupSubModeButton("#eraser-button", "-pin", Mode.ERASER_PIN, () => {
    mEraserController.setActive(true);
  });
  setupButtonTooltip("#eraser-button-pin", "ピンのみを消去");
  setupSubModeButton("#eraser-button", "-image", Mode.ERASER_IMAGE, () => {
    mEraserController.setActive(true);
  });
  setupButtonTooltip("#eraser-button-image", "画像のみを消去");
  ...
  setupButtonTooltip("#color-bucket-button", "すべてのものに色を付ける！");
  ...
  setupSubModeButton(
    "#color-bucket-button",
    "-eyedropper",
    Mode.COLOR_BUCKET_EYEDROPPER,
    setEyeDropperActive,
  );
  setupButtonTooltip(
    "#color-bucket-button-eyedropper",
    "色付け可能なものから色をコピー",
  );
  ...
  setupButtonTooltip(
    "#color-bucket-button-color-picker",
    "色付けに使用する色を選択",
  );
  ...
  setupButtonTooltip("#panning-button", "メインビューとレンズビューをパンする");
  ...
  setupButtonTooltip("#link-button", "データをタイムラインに添付");
  $("#link-button").on("pointerenter", () => {
    mMousedOverLinkButton = true;
  });
  $("#link-button").on("pointerout", () => {
    mMousedOverLinkButton = false;
  });
  ...
  $("#dynamic-normals-axis-button").on("click", () => {
    ...
    mModelController.updateAxisDataAlignment(
      mSelectedAxisId,
      DataDisplayAlignments.FIXED,
    );
    pushVersion();
  ...
  setupButtonTooltip(
    "#fixed-normals-axis-button",
    "ラインに対するデータの配置を変更する",
  );
  $("#delete-axis-button").on("click", deleteSelected);
  setupButtonTooltip(
    "#delete-axis-button",
    "このセット内のすべてのデータポイントのリンクを解除",
  );
  ...
  $("#add-datasheet-button").on("click", () => {
    ...
    for (let i = 0; i < 3; i++) {
      ...
      dataRow.index = i;
      dataRow.dataCells.push(
        new DataStructs.TimeCell("", newTable.dataColumns[0].id),
      );
      for (let j = 1; j < newTable.dataColumns.length; j++) {
        dataRow.dataCells.push(
          new DataStructs.DataCell(
            DataTypes.UNSPECIFIED,
            "",
            newTable.dataColumns[j].id,
          ),
        );
      }
  ...
  setupButtonTooltip("#add-datasheet-button", "新しいデータシートを追加");
  ...
  setupButtonTooltip("#upload-datasheet-button", "CSVデータシートをアップロード");
  ...
  function setupSubModeButton(buttonId, subButtonAppendix, mode, callback) {
    $(buttonId + subButtonAppendix).on("click", () => {
      if (mMode == mode) {
        ...
        modeImg.css(
          "background-color",
          $(buttonId + subButtonAppendix).css("background-color"),
        );
        modeImg.addClass("mode-indicator");
        $("#mode-indicator-div").append(modeImg);
        $("#mode-indicator-div").show();
    ...
    setTimeout(() => {
      ...
      if (top + height > window.innerHeight) {
        ...
        $("#sub-menu-wrapper").css(
          "bottom",
          window.innerHeight -
            ($(buttonId).offset().top + $(buttonId).height()),
        );
      } else {
  ...
  function setDefaultMode() {
    ...
    modeImg.css(
      "background-color",
      $("#selection-button").css("background-color"),
    );
    modeImg.addClass("mode-indicator");
    $("#mode-indicator-div").append(modeImg);
    $("#mode-indicator-div").show();

    $("#selection-button-sub-menu").css(
      "top",
      $("#selection-button").offset().top,
    );
    $("#selection-button-sub-menu").css(
      "left",
      $("#selection-button").offset().left -
        $("#selection-button-sub-menu").outerWidth() -
        10,
    );
  ...
  function toggleColorPicker(e) {
    if ($("#color-picker-div").is(":visible")) {
      ...
      $("#color-picker-div").css(
        "top",
        e.pageY - $("#color-picker-div").height(),
      );
      $("#color-picker-div").css(
        "left",
        e.pageX - $("#color-picker-div").width(),
      );
  ...
  function showLineTime(timelineId, screenCoords) {
    ...
    let pointOnLine = PathMath.getClosestPointOnPath(
      svgCoords,
      timeline.points,
    );
    ...
    let time = mModelController
      .getModel()
      .mapLinePercentToTime(timelineId, pointOnLine.percent);
    let message = mModelController.getModel().hasTimeMapping(timelineId)
      ? DataUtil.getFormattedDate(time)
      : "時間の割合: " + Math.round(time * 100) + "%";
  ...
  function showLensView(timelineId, percent) {
    ...
    if (!timeline)
      console.error(
        "Bad state! tried to show highlight for non-existant line: " +
          timelineId,
      );
